...
int run_length_decode(int *code, int size, int *msg)
{
    ...
    for (i = 0; i < size; i++) {
        if (code[i] != 0) {
            ...
            // 0 is the last element or the length of 1 is less than 3 indicates that the
            // encoded sequence is wrong.
            if (i + 1 >= size || code[i+1] < 3) {
                printf("there is an error in the RLE decode process, abort");
                return -1;
            }
...
int run_length_encode(int *msg, int size, int *code)
{
    ...
    while (i < size) {
        if (msg[i] != 1) {
            ...
            int k = i + 1;
            while (k < size && msg[k] == 1)k++;
            int len = k - i;
            //the length must be greater then 3 
            if (len >= 3) {
                code[j++] = 0;
                code[j++] = len;
                i = k;
...
int check_run_length(int *msg, int size, int *code)
{
    ...
    int len = run_length_decode(code, size, msg2);
    if (len < 0) {
        val = 0;
    } else {
        for (i = 0; i < len; i++) {
